...
export default function AdminPage() {
  ...
  const [expenseCategorySaving, setExpenseCategorySaving] = useState(false);
  ...
  const fetchData = async () => {
    try {
      const [recordsRes, statsRes, categoriesRes] = await Promise.all([
        ...
        fetch("/api/finance/admin/expense-categories"),
      ]);
      ...
      if (categoriesResult.success && Array.isArray(categoriesResult.data)) {
        const sorted = [...(categoriesResult.data as ExpenseCategoryConfigItem[])].sort(
  ...
  const handleSaveExpenseCategories = async () => {
    ...
    try {
      ...
      setExpenseCategorySaving(false);
    }
  };
      ...
      <Card className="rounded-2xl border border-border/60 shadow-sm">
        ...
        <CardContent className="space-y-2 px-4 pb-4 pt-0 sm:px-5">
          ...
          <div className="flex justify-end pt-1">
            ...
            >
              {expenseCategorySaving ? "保存中..." : "保存类别配置"}
            </Button>
          </div>
        </CardContent>
      </Card>
