...
## UI 样式规范 (核心要求)
1.  **布局控制**: 必须适配 **16:9 的屏幕比例**，固定宽高以确保在一个屏幕内呈现全部内容，不需要考虑屏幕自适应，**绝对不能出现滚动条**。
2.  **色彩方案**: 
    -   页面底色：#F9FAFB (极浅灰)。
    -   卡片背景：纯白 #FFFFFF。
...
4.  **核心组件**:
    -   **标题栏**: 大标题加粗 + 下方灰色副标题。
    -   **核心板块 (栅格布局：可以根据实际情况调整)**: 
        -   卡片顶部：左侧显示彩色图标 + 标题，背景色为对应主题色的极淡色（如淡蓝、淡橘），右侧显示小尺寸浅色背景标签,
        -   卡片内部：上半部分为**加粗的核心观点**。
        -   中间分隔：**浅灰色虚线分隔符** (border-top: 1px dashed #E5E6EB)。
        -   下半部分：紧凑的无序号列表，使用实心点作为子弹头。
        -   卡片高度不需要因为屏幕高度的变化而变化
...
- 副标题：[时间 | 参会人 | 目标]
- 核心讨论事项 (根据实际情况调整模块数量):
  - 模块 A: [标题] / [标签] / [一句话核心观点] / [3-4个要点列表]
  - 模块 B: [标题] / [标签] / [一句话核心观点] / [3-4个要点列表]
  ---
- 下一步行动 / Todo (根据实际情况调整)
- 核心结论: [底部总结文字]
